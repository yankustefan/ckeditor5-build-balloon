    ...
    for (const entry of changes) {
        ...
        if (entry.type === 'attribute' && entry.attributeKey !== TEXT_SPAN) {
            ...
            if (entry.attributeNewValue && entry.attributeOldValue === null) {
                ...
                wasChanged = setTextSpanAttribute(nodeBefore, writer);
                wasChanged = setTextSpanAttribute(nodeAfter, writer);
...
function setTextSpanAttribute(node, writer) {
    if (isTextLikeNode(node) && hasNoAttributes(node)) {
        writer.setAttribute(TEXT_SPAN, true, node);
        return true
    }
    return false;
